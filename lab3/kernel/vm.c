...
#include "defs.h"
#include "fs.h"
#define CUSTOM_VA     0x70000000UL
#define PLV0       (0L << 2)
#define MAP_VA   0x000090000ULL   /* 0x90000 → 4KiB 对齐的低位虚拟地址 */
#define MAP_PA   0x000020000ULL   /* 物理页基址，页对齐 */
#define MAP_SZ   PGSIZE           /* 只映射一页 */
void handle_tlbr();
void test_kernel_paging()
{
    uint64 test_pa = 0x20000;
    void *win_addr = (void*)(0x9000000000000000UL | test_pa);
    printfa("   Physical address: 0x%lx\n", test_pa);
    printfa("   Window address:   0x%lx\n", (uint64)win_addr);
    
    char *str_ptr = (char*)((uint64)win_addr);
    const char *message = "Hello from window mapping!";
    for (int i = 0; message[i] != '\0'; i++) {
        str_ptr[i] = message[i];
    }
    printfa("   Written: \"%s\"\n", message);
    
printfa("   Read back via window: \"");
    for (int i = 0; str_ptr[i] != '\0' && i < 50; i++) {
        printfa("%c", str_ptr[i]);
    }
    printfa("\"\n");
     ...
     uint64 map_va = 0x50000;
 
int a=mappages(pgtbl, map_va, PGSIZE, test_pa, PTE_P |PTE_V | PTE_W | PTE_MAT |PTE_D |PLV0);
if(a < 0) {
    printfa("Failed to map pages\n");
    kfree(pgtbl);
    return;
}else
{
	printfa("%d\n",a);
	pte_t *pte = walk(pgtbl, map_va, 0);
        printfa("pte = 0x%lx\n", pte ? *pte : 0);
}

  w_csr_pgdl((uint64)pgtbl& 0xFFFFFFFFULL);
    asm volatile("invtlb 0x0, $zero, $zero");
  //  tlbinit();
   

   printfa("read:%s", (char*)map_va);
...
vminit(void)//todo
{
  ...
  //1
  printfa("kpgtbl:0x%lx\n",kpgtbl);
  memset(kpgtbl, 0, PGSIZE);
  proc_mapstacks(kpgtbl);
  
  
  //uint64 kpgtbl_a = (uint64)kpgtbl;
    kpgtbl = (pagetable_t)((uint64)kpgtbl & 0xFFFFFFFFUL);
    
    
    //kpgtbl = (pagetable_t)kpgtbl_a;
printfa("kpgtbl:0x%lx\n",kpgtbl);
